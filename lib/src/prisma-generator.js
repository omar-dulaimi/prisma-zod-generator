...
const plimit_1 = __importDefault(require("./helpers/plimit"));
...
async function generateObjectSchemas(inputObjectTypes) {
    console.log('generateObjectSchemas', plimit_1.default);
    const limit = (0, plimit_1.default)(1); // Limit to 10 concurrent executions
    console.log('limit :>> ', limit);
    const tasks = inputObjectTypes.map((inputObjectType, index) => {
        return limit(async () => {
            const startTime = Date.now();
            console.log(`Starting processing for ${inputObjectType === null || inputObjectType === void 0 ? void 0 : inputObjectType.name} at ${new Date(startTime).toISOString()}`);
            const fields = inputObjectType === null || inputObjectType === void 0 ? void 0 : inputObjectType.fields;
            const name = inputObjectType === null || inputObjectType === void 0 ? void 0 : inputObjectType.name;
            const transformer = new transformer_1.default({
                name,
                fields,
            });
            ...
            console.log(`Finished processing for ${inputObjectType === null || inputObjectType === void 0 ? void 0 : inputObjectType.name} at ${new Date(endTime).toISOString()}`);
            console.log(`Processing time for ${inputObjectType === null || inputObjectType === void 0 ? void 0 : inputObjectType.name}: ${endTime - startTime}ms`);
        });
    });
