...
export class MultiProviderTestRunner {
   ...
   * Run provider-specific tests
   */
  private async runProviderSpecificTests(
  ...
  private async runPerformanceTests(config: ProviderConfig, testSuite: TestSuite): Promise<void> {
    ...
    const performanceTests = ['Schema Validation Speed', 'Memory Usage', 'File Size', 'Load Time'];
   ...
   */
  private async runProviderSpecificTest(
    config: ProviderConfig,
    testCase: TestCase,
  ): Promise<TestResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      return {
        ...
        errors: validationResults
          .filter((r) => !r.passed)
          .map((r) => r.error || 'Validation failed'),
   ...
   */
  private async validateCoreTest(
    config: ProviderConfig,
    testName: string,
    schemas: string[],
  ): Promise<boolean> {
    ...
    switch (testName) {
      case 'Basic Types':
        return schemas.some(
          (schema) => schema.includes('z.string()') && schema.includes('z.number()'),
        );
  ...
  private async validateTestCase(
    ...
    testCase: TestCase,
    schemas: string[],
  ): Promise<Array<{ passed: boolean; error?: string; warning?: string }>> {
  ...
  private printSummary(report: TestReport): void {
    ...
    for (const suite of report.testSuites) {
      ...
      console.log(
        `  ${status} ${suite.name}: ${suite.summary.passedTests}/${suite.summary.totalTests} (${suite.summary.coverage.toFixed(1)}%)`,
      );
    }
