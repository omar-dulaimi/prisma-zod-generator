...
export class PrismaTypeMapper {
  ...
  mapFieldToZodSchema(field: DMMF.Field, model: DMMF.Model): FieldTypeMappingResult {
    ...
    try {
      ...
      result.additionalValidations.push(`// Warning: Failed to map type ${field.type}, using unknown`);
    }
  ...
  private mapDecimalType(field: DMMF.Field, result: FieldTypeMappingResult): void {
    ...
    switch (this.config.decimalMode) {
      ...
      case 'number':
        ...
        if (decimalConfig.validatePrecision && decimalConfig.maxPrecision && decimalConfig.maxPrecision > 15) {
    ...
    result.requiresSpecialHandling = true;
    result.additionalValidations.push(`// Decimal field mapped as ${this.config.decimalMode} with enhanced validation`);
  }
   ...
   */
  private mapObjectType(field: DMMF.Field, model: DMMF.Model, result: FieldTypeMappingResult): void {
   ...
   */
  private applyDatabaseSpecificOptionalityRules(field: DMMF.Field, result: FieldOptionalityResult): void {
   ...
   */
  private applyInlineValidations(field: DMMF.Field, result: FieldTypeMappingResult, modelName: string = 'Unknown'): void {
    ...
    try {
      ...
      if (!parseResult.isValid || parseResult.annotations.length === 0) {
        if (!parseResult.isValid && parseResult.parseErrors.length > 0) {
          result.additionalValidations.push(`// @zod parsing errors: ${parseResult.parseErrors.join(', ')}`);
        }
        return;
      }
  ...
  private buildFieldProperties(metadata: JSDocMetadata): string[] {
    ...
    if (metadata.validations.optionalityReason && metadata.validations.optionalityReason !== 'required') {
  ...
  private addPostgreSQLOptimizations(field: DMMF.Field, result: FieldTypeMappingResult): void {
    ...
    if (field.type === 'Json') {
      result.databaseSpecific?.optimizations.push('PostgreSQL JSONB provides better performance than JSON');
    }
  }
  ...
  private addMySQLOptimizations(field: DMMF.Field, result: FieldTypeMappingResult): void {
    if (field.type === 'String' && field.isList) {
      result.databaseSpecific?.optimizations.push('Consider using separate table for array data in MySQL');
    }
  }
  ...
  static validateTypeMapping(config: Partial<TypeMappingConfig>): string[] {
    ...
    if (config.provider && !['postgresql', 'mysql', 'sqlite', 'sqlserver', 'mongodb'].includes(config.provider)) {
  ...
  generateValidationReport(collection: SchemaCollection): SchemaValidationReport {
    ...
    if (collection.generationSummary.enhancedFields < collection.generationSummary.totalFields * 0.1) {
