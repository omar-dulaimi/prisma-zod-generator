 ...
 */
export function computeEnumImportPath(fromVariantDirAbs: string, enumsDirAbs: string, enumName: string): string {
 ...
 */
export function generateEnumSchemaImportLines(enumNames: string[], depthFromVariantDir = 2): string {
  if (!enumNames.length) return '';
  const upSegments = Array(depthFromVariantDir).fill('..').join('/');
  return enumNames.map(e => `import { ${e}Schema } from '${upSegments}/enums/${e}.schema';`).join('\n');
}
