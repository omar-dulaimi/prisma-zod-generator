...
async function isLikelyGeneratedFile(filePath: string): Promise<boolean> {
  try {
    ...
    // Only signatures specific to prisma-zod-generator output. Generic markers
    // (PrismaClient, export const, Prisma.) match files emitted by the
    // prisma-client generator or user code sharing the output directory, and
    // deleting those broke shared-output setups (issue #365).
...
// Files emitted by Prisma's own client generators (prisma-client /
// prisma-client-js with custom output). Never delete these during smart
// cleanup: users commonly point both generators at the same directory (#365).
const PRISMA_CLIENT_OUTPUT_FILES = new Set([
  'client.ts',
  'client.d.ts',
  'models.ts',
  'enums.ts',
...
export async function performSmartCleanup(outputPath: string): Promise<void> {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (file.isFile()) {
        if (file.name === MANIFEST_FILENAME) continue;
        if (PRISMA_CLIENT_OUTPUT_FILES.has(file.name)) {
          logger.debug(
            `[safeOutputManagement] Preserved Prisma client generator file: ${file.name}`,
          );
          continue;
        }
