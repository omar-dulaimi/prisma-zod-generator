...
describe('Custom Prisma Client output path integration', () => {
  it('emits relative Prisma import when client output customized', async () => {
    ...
    try {
      ...
      const schema = base.replace('generator client {', `generator client {\n  output = "${clientRel}"`);
  ...
  it('emits relative Prisma import in CRUD operation schema files', async () => {
    ...
    try {
      ...
      const crudFile = schemaFiles.find(f => /findMany.*\.schema\.ts$/.test(f))
