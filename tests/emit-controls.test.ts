...
describe('Explicit emission controls (emit.*)', () => {
  it('should emit only pure models when crud, objects, results disabled', async () => {
...
`;
      ...
      expect(existsSync(join(base, 'variants', 'pure', 'User.pure.ts')) || existsSync(join(base, 'variants', 'pure', 'User.model.ts'))).toBe(true);
      expect(existsSync(join(base, 'objects'))).toBe(false);
      expect(existsSync(join(base, 'results'))).toBe(false);
      // CRUD file pattern (findMany)
      const crudFindMany = ['findManyUser.schema.ts','UserFindMany.schema.ts','findManyUserArgs.schema.ts'].map(f=>join(base,f)).some(p=>existsSync(p));
  ...
  it('should skip enums when emit.enums=false', async () => {
...
`;
  ...
  it('should emit crud but not results when configured', async () => {
...
`;
      ...
      await env.runGeneration();
      const base = join(env.outputDir, 'schemas');
      expect(existsSync(join(base, 'UserFindMany.schema.ts')) || existsSync(join(base,'findManyUser.schema.ts'))).toBe(true);
      expect(existsSync(join(base, 'results'))).toBe(false);
