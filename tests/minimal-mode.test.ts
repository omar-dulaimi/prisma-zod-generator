...
describe('Minimal Mode Tests', () => {
  describe('Minimal Mode Configuration', () => {
    it('should generate only essential CRUD operations in minimal mode', async () => {
      ...
      try {
        ...
        essentialOperations.forEach(operation => {
          ...
          expect(existsSync(filePath), `Essential operation should exist: ${operation}`).toBe(true);
        });
        ...
        advancedOperations.forEach(operation => {
          const filePath = join(schemasDir, operation);
          expect(existsSync(filePath), `Advanced operation should NOT exist in minimal mode: ${operation}`).toBe(false);
        });
    ...
    it('should exclude complex input types in minimal mode', async () => {
      ...
      try {
        ...
        complexInputTypes.forEach(inputType => {
          ...
          expect(existsSync(filePath), `Complex input type should NOT exist in minimal mode: ${inputType}`).toBe(false);
        });
        ...
        basicInputTypes.forEach(inputType => {
          const filePath = join(objectsDir, inputType);
          expect(existsSync(filePath), `Basic input type should exist in minimal mode: ${inputType}`).toBe(true);
        });
    ...
    it('should generate minimal schemas for multiple models', async () => {
      ...
      try {
        ...
        models.forEach(model => {
          essentialOps.forEach(op => {
            ...
            expect(existsSync(filePath), `${model} should have essential operation: ${fileName}`).toBe(true);
          });

          // Should NOT have advanced operations
          const advancedOps = ['aggregate', 'groupBy', 'upsertOne', 'createMany', 'updateMany', 'deleteMany'];
          advancedOps.forEach(op => {
            const fileName = `${op}${model}.schema.ts`;
            const filePath = join(schemasDir, fileName);
            expect(existsSync(filePath), `${model} should NOT have advanced operation: ${fileName}`).toBe(false);
          });
        });
  ...
  describe('Minimal Mode vs Full Mode Comparison', () => {
    it('should generate significantly fewer files in minimal mode', async () => {
      ...
      try {
        ...
        const fullSchema = baseSchema.replace('./generated/schemas', fullModeEnv.outputDir + '/schemas')
          .replace('provider = "node ./lib/generator.js"', 'provider = "node ./lib/generator.js"\n  config   = "./config.json"');
        ...
        const minimalSchema = baseSchema.replace('./generated/schemas', minimalModeEnv.outputDir + '/schemas')
          .replace('provider = "node ./lib/generator.js"', 'provider = "node ./lib/generator.js"\n  config   = "./config.json"');
    ...
    it('should maintain functionality while reducing complexity', async () => {
    ...
    it('should generate smaller bundle sizes in minimal mode', async () => {
  ...
  describe('Minimal Mode with Relationships', () => {
    it('should handle relationships in minimal mode', async () => {
      ...
      try {
        ...
        complexRelationInputs.forEach(inputType => {
          const filePath = join(objectsDir, inputType);
          expect(existsSync(filePath), `Complex relation input should NOT exist in minimal mode: ${inputType}`).toBe(false);
        });
    ...
    it('should simplify relationship handling in minimal mode', async () => {
...
`;
        ...
        complexInputs.forEach(inputType => {
          const filePath = join(objectsDir, inputType);
          expect(existsSync(filePath), `Complex nested input should NOT exist: ${inputType}`).toBe(false);
        });
  ...
  describe('Minimal Mode Configuration Options', () => {
    it('should respect minimal mode flag in configuration', async () => {
      ...
      try {
        ...
        includeSelectTypes.forEach(inputType => {
          const filePath = join(objectsDir, inputType);
          expect(existsSync(filePath), `Include/Select type should NOT exist in minimal mode: ${inputType}`).toBe(false);
        });
    ...
    it('should allow customization of minimal operations set', async () => {
      ...
      try {
        ...
        allowedOperations.forEach(operation => {
          const filePath = join(schemasDir, operation);
          expect(existsSync(filePath), `Custom minimal operation should exist: ${operation}`).toBe(true);
        });
  ...
  describe('Minimal Mode Performance', () => {
    it('should have faster generation times in minimal mode', async () => {
    ...
    it('should use less memory in minimal mode', async () => {
  ...
  describe('Minimal Mode Validation', () => {
    it('should validate generated schemas are syntactically correct', async () => {
    ...
    it('should maintain type safety in minimal mode schemas', async () => {
