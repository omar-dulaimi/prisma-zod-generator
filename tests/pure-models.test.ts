...
describe('Pure Model Schema Generation Tests', () => {
  describe('Prisma Type Mapping', () => {
    it('should map all Prisma field types to appropriate Zod schemas', async () => {
    ...
    it('should handle field optionality correctly', async () => {
...
`;
  ...
  describe('Inline @zod Validation Application', () => {
    it('should apply @zod validations to pure model schemas', async () => {
    ...
    it('should combine Prisma type mapping with @zod validations', async () => {
...
`;
        ...
        if (existsSync(productModelPath)) {
          ...
          expect(content).toMatch(/rating.*z\.number\(\).*\.min\(0\).*\.max\(5\).*\.optional\(\)/);
  ...
  describe('JSDoc Generation', () => {
    it('should generate JSDoc documentation from Prisma field comments', async () => {
...
`;
    ...
    it('should handle complex documentation patterns', async () => {
...
`;
  ...
  describe('Complex Type Handling', () => {
    it('should handle Decimal, JSON, Bytes, and DateTime types correctly', async () => {
...
`;
    ...
    it('should handle enum types properly', async () => {
...
`;
  ...
  describe('Schema Composition Logic', () => {
    it('should compose type mapping, optionality, validation, and documentation into cohesive schemas', async () => {
...
`;
        ...
        if (existsSync(userModelPath)) {
          ...
          expect(content).toMatch(/email:[\s\S]*z\.string\(\)[\s\S]*\.email\(\)[\s\S]*\.toLowerCase\(\)/);
          
          // Should compose optionality with validation for name
          expect(content).toMatch(/name:[\s\S]*z\.string\(\)[\s\S]*\.min\(2\)[\s\S]*\.max\(50\)[\s\S]*\.trim\(\)[\s\S]*\.optional\(\)/);
          
          // Should compose validation with optionality for age
          expect(content).toMatch(/age:[\s\S]*z\.number\(\)\.int\(\)[\s\S]*\.min\(0\)[\s\S]*\.max\(120\)[\s\S]*\.optional\(\)/);
          ...
          expect(content).toMatch(/preferences:[\s\S]*z\.record\(z\.string\(\)\)[\s\S]*\.optional\(\)/);
    ...
    it('should maintain proper schema structure and exports', async () => {
  ...
  describe('Pure Model Integration with Other Features', () => {
    it('should work correctly with field exclusions', async () => {
...
`;
    ...
    it('should integrate with schema variants', async () => {
