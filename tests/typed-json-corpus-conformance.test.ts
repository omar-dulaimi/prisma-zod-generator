...
async function generateCorpus(corpus: CorpusFile): Promise<GeneratedCorpus> {
  const envName = `pjtg-${corpus.file.replace(/\.prisma$/, '')}`;
  ...
  /* --------------------------------------------------------------- normal */
  {
    ...
    cases: [
      ...
      ...listWrapperCases('normal.test-d.ts'),
      {
        ...
        gap: "PZG's groupBy result marks _count/_avg/_sum/_min/_max optional but not nullable, so upstream's explicit nulls are rejected",
      },
      {
        target: 'results/ModelGroupByResult',
        expect: 'accept',
...
describe('PJTG corpus conformance: generate, then execute upstream assertions', () => {
  for (const corpus of CORPUS) {
    describe(corpus.file, () => {
      ...
      beforeAll(async () => {
        generated = await generateCorpus(corpus);
      }, GENERATION_TIMEOUT);

      for (const testCase of corpus.cases) {
        const label = `${testCase.expect} ${testCase.target}: ${testCase.from}`;

        it(testCase.gap ? `KNOWN GAP - ${label}` : label, async () => {
          ...
          if (testCase.gap) {
            ...
            expect(
              matched,
              `this row is recorded as a known gap (${testCase.gap}) but PZG now matches upstream. Remove the gap.`,
            ).toBe(false);
            return;
          }
...
describe('PJTG corpus conformance: the score', () => {
  const all = CORPUS.flatMap((corpus) => corpus.cases.map((c) => ({ file: corpus.file, ...c })));
  ...
  it('pins the tallies, so a gap cannot be added quietly', () => {
    ...
    const tally = {
      cases: all.length,
      matching: all.length - gaps.length,
      gaps: gaps.length,
    };
