...
describe('@zod.custom.use common patterns', () => {
  it('handles union, record, optional field, and multiline override', async () => {
...
`;
  ...
  expect(content).toMatch(/settings: z\.record\(z\.string\(\)\)\.default\("\{\}"\)/);
      ...
      expect(content).toMatch(/payload2: z\.union\(\[z\.string\(\), z\.number\(\), z\.boolean\(\)\]\)\.optional\(\)/);
  // items: array of object override with default
  expect(content).toMatch(/items: z\.array\(z\.object\(\{ a: z\.string\(\), b: z\.number\(\)\.optional\(\) \}\)\)\.default\("\[\]"\)/);
  ...
  expect(content).toMatch(/event: z\.union\(\[.*type: z\.literal\("a"\).*type: z\.literal\("b"\).*\]/s);
