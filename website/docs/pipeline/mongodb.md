...
A field whose type is a MongoDB [composite type](https://www.prisma.io/docs/orm/prisma-schema/data-model/models#defining-composite-types) — a `type` block, not a `model` — has the same DMMF `kind: 'object'` as a real relation, but Prisma never sets `relationName` for one, since a composite isn't a relation.
...
CRUD/input/where schemas need no special handling: Prisma's own DMMF pre-expands every composite into ordinary `inputObjectTypes`/`outputObjectTypes` entries — `AddressCreateInput`, `AddressWhereInput`, `AddressObjectEqualityInput`, and so on under `objects/` — that the generic object-schema walker already covers.

The pure, result and input **variant** schemas (`variants/`) build each field's expression directly from `model.fields` rather than from those pre-expanded DMMF types, though, so they reference the composite's `ObjectEqualityInput` schema — the one shaped like the composite's own fields, used elsewhere for equality filtering — instead:

```ts
// variants/pure/Customer.pure.ts
...
export const CustomerModelSchema = z.object({
  id: z.string(),
  name: z.string(),
  address: AddressObjectEqualityInputObjectSchema,
}).strict();
```

A list of composites (`addresses Address[]`) gets `.array()` appended the same way an enum list does.
