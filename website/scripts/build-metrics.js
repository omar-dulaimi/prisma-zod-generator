...
async function extractMetrics() {
  try {
    ...
    const downloadBadgeMatch = readmeContent.match(/https:\/\/img\.shields\.io\/npm\/dw\/prisma-zod-generator/);
    ...
    try {
      ...
      const response = await fetch('https://api.npmjs.org/downloads/point/last-week/prisma-zod-generator');
    ...
    try {
      ...
      const fetch = (await import('node-fetch')).default;
      const response = await fetch('https://api.github.com/repos/omar-dulaimi/prisma-zod-generator');
      const data = await response.json();
